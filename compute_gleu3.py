...
# compute_gleu
#
# This script calls gleu3.py to calculate the GLEU score of a sentence, as
            ...
            if args.debug:
                ...
                for r in range(len(args.reference)):
                    if stats_by_ref[r] is None:
                        stats_by_ref[r] = [s for s in
                                           gleu_calculator.gleu_stats(
                                               i, r_ind=r)]

                print(f'{i} '
                      f'{" ".join(get_gleu_stats([gleu_calculator.gleu(stats, smooth=True) for stats in stats_by_ref]))}')

        if args.debug:
            print('\n==== Overall score =====\nMean Stdev 95%CI GLEU\n' +
                  ' '.join(get_gleu_stats([gleu_calculator.gleu(stats)
